...
class NeoBDMScraper:
    def __init__(self, username, password, headless=False, mirror_output_dir="/Users/albert/Documents/Finances/projects/02_alpha_research/alpha_research/data/pure_raw/neo_bdm/08_jun_2026"):
        """Inisialisasi scraper dengan kredensial login"""
        self.username = username
        self.password = password
        self.driver = None
        self.wait = None
        self.output_dir = "/Users/albert/Documents/Finances/data/raw/market_data/bandarmology/08_jun_2026"
        self.mirror_output_dir = mirror_output_dir
    ...
    def reset_pagination_to_first_page(self):
        """Reset pagination ke halaman 1 untuk preset yang baru dipilih"""
        try:
            # Tunggu sampai elemen pagination tersedia
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, "#table-custom-pagination .tabulator-page"))
            )
            
            # Cari tombol halaman 1 berdasarkan data-page="1"
            try:
                first_page_btn = self.driver.find_element(By.CSS_SELECTOR, '#table-custom-pagination .tabulator-page[data-page="1"]')
                # Cek apakah tombol aktif (current page) bukan 1
                if 'active' not in first_page_btn.get_attribute('class'):
                    first_page_btn.click()
                    print("  ✓ Reset pagination to page 1")
                    time.sleep(2)  # Tunggu tabel reload
                else:
                    print("  ✓ Already on page 1")
            except NoSuchElementException:
                # Jika tidak ada data-page="1", coba tombol "First"
                try:
                    first_btn = self.driver.find_element(By.CSS_SELECTOR, '#table-custom-pagination .tabulator-page[data-page="first"]')
                    first_btn.click()
                    print("  ✓ Reset pagination via First button")
                    time.sleep(2)
                except NoSuchElementException:
                    print("  ⚠️ Could not find pagination button to reset to page 1")
    ...
    def scrape_all_pages_dynamic(self, preset_value):
        ...
        dengan reset pagination ke halaman 1 terlebih dahulu.
        """
        ...
        self.reset_pagination_to_first_page()
    ...
    def mirror_file(self, filepath):
        """Copy saved output to secondary data/raw path."""
        if not self.mirror_output_dir or not filepath or not os.path.exists(filepath):
            return None

        os.makedirs(self.mirror_output_dir, exist_ok=True)
        mirror_path = os.path.join(self.mirror_output_dir, os.path.basename(filepath))

        if os.path.abspath(filepath) != os.path.abspath(mirror_path):
            shutil.copy2(filepath, mirror_path)
    ...
    def save_to_csv(self, data, filename):
        ...
        try:
            ...
            mirror_path = self.mirror_file(filepath)
