...
OUTPUT_DIR = ["../data/news/macro_news/idx", "/Users/albert/Documents/Finances/projects/02_alpha_research/alpha_research/data/pure_raw/news/macro_news/idx"]
...
class IDXChannelMacroScraper:
    ...
    def save_results(self, results, target_date):
        ...
        filename = f"idxchannel_macro_{target_date.strftime('%Y%m%d')}_{timestamp}.json"
...
def main():
    print("=" * 70)
    print(f"IDX CHANNEL MACRO NEWS SCRAPER  ({KANAL_NAME} | idkanal={KANAL_ID})")
    print("=" * 70)
    ...
    for day_idx, target_date in enumerate(target_dates, start=1):
        ...
        try:
            ...
            if results:
                timestamp = datetime.now().strftime("%H%M%S")
                partial_name = f"idxchannel_macro_partial_{target_date.strftime('%Y%m%d')}_{timestamp}.json"
