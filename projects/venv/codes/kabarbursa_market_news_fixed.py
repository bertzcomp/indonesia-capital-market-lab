...
OUTPUT_DIR = ["../data/news/market_news/kb", "/Users/albert/Documents/Finances/projects/02_alpha_research/alpha_research/data/pure_raw/news/market_news/kb"]
...
class KabarBursaScraper:
    ...
    def save_results(self, results, target_date):
        ...
        filename = f"kabarbursa_{target_date.strftime('%Y%m%d')}_{timestamp}.json"
...
def main():
    print("=" * 70)
    print("KABARBURSA MARKET NEWS SCRAPER  (summary + detail sekaligus)")
    print("=" * 70)
    ...
    for day_idx, target_date in enumerate(target_dates, start=1):
        ...
        try:
            ...
            if results:
                timestamp = datetime.now().strftime("%H%M%S")
                partial_name = f"kabarbursa_partial_{target_date.strftime('%Y%m%d')}_{timestamp}.json"
