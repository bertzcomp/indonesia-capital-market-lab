...
OUTPUT_DIR = "../data/news/market_news/idx"
...
class IDXChannelScraper:
    ...
    def save_results(self, results, target_date):
        ...
        filename = os.path.join(
            OUTPUT_DIR,
            f"idxchannel_{target_date.strftime('%Y%m%d')}_{timestamp}.json"
        )
...
def main():
    print("=" * 70)
    print("IDX CHANNEL DAILY SCRAPER  (listing + detail sekaligus)")
    print("=" * 70)
    ...
    try:
        ...
        if results:
            timestamp = datetime.now().strftime("%H%M%S")
            partial_file = os.path.join(OUTPUT_DIR, f"idxchannel_partial_{timestamp}.json")
            os.makedirs(OUTPUT_DIR, exist_ok=True)
