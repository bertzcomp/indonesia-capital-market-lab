...
def scrape_broker_summary(date_str, code, session, headers_post):
    ...
    try:
        ...
        # Rename kolom pertama dari tabel (kode broker) menjadi broker_code
        if len(df.columns) > 0:
...
def main():
    ...
    for date_str in date_list:
        # date_str sudah dalam format YYYY-MM-DD — langsung pakai sebagai nilai kolom 'date'
        ...
        for code, df in data_frames.items():
            # Kolom urutan: date | ticker | broker_code | ... kolom lain dari tabel
            df.insert(0, 'ticker', code)       # kode saham — sebelumnya 'StockCode'
            df.insert(0, 'date',   date_str)   # tanggal format YYYY-MM-DD
            all_parts_df.append(df)

        final_df = pd.concat(all_parts_df, ignore_index=True)
        final_df.to_csv(filepath, index=False)
        print(f"[Part {part}] {date_str} → {filename} ({len(data_frames)} saham, {len(final_df):,} rows)")
